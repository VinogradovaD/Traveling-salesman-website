...
function nearestCity(currentCity, currentPath, distanceMatrix) {
  ...
  for (let i = 0; i < distanceMatrix.length; i++) {
    if (
      distanceMatrix[i][currentCity - 1] < min &&
      !!distanceMatrix[i][currentCity - 1] &&
      existInArray(i + 1, currentPath) !== -1
    ) {
...
function generateAnswer(result) {
  return `Маршрут: ${result.path.join("->")}<br>Длина маршрута: ${
    result.distance
...
function drawGraph() {
  (function ($) {
    ...
    $(document).ready(function () {
      ...
      for (let i = 0; i < cityCount; i++) {
        sys.addEdge(
          sys.getNode(result.path[i]),
          sys.getNode(result.path[i + 1]),
          { name: matrix[result.path[i + 1] - 1][result.path[i] - 1] }
        );
      }
    });
  })(this.jQuery);
